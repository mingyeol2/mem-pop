...
- [ ] **Task 0.1: 데이터 타입 및 스키마 정의 (`types/mem-pop.ts`)**
...
- [ ] **Task 1.1: 시스템 프롬프트 엔지니어링 (`lib/prompts.ts`)**
...
- [ ] **Task 1.2: AI 요청 핸들러 및 JSON 스키마 검증기 (`app/api/generate/route.ts` 또는 클라이언트 API)**
...
- [ ] **Task 2.1: 헤더 및 단일 인풋 컴포넌트 (`components/keyword-input.tsx`)**
...
| :--- | :--- | :--- | :---: |
| **원페이지 UI 구성** | PRD 3, 6.1 | 타이틀, 인풋, 버튼, 결과 카드(스토리+퀴즈+다시생성)가 단일 화면에 유기적으로 배치됨 | 🔲 대기 |
| **3줄 연상 암기 생성** | PRD 4.2, 6.2 | 키워드 입력 후 5초 이내에 3단계 연상 팁 정상 렌더링 | 🔲 대기 |
| **3지선다 1초 퀴즈** | PRD 4.3, 6.2 | 정답/오답 즉각 피드백, 보기 비활성화 및 해설 노출 | 🔲 대기 |
| **빈 값 유효성 검증** | PRD 5.1, 6.3 | 버튼 비활성화, 레드 보더 및 안내 문구 작동 | 🔲 대기 |
| **30자 제한 & 다중단어 경고**| PRD 5.2, 6.3 | 30자 초과 타이핑 차단, 2개 이상 구분자 입력 시 경고 표시 | 🔲 대기 |
| **네트워크 오류 1회 Auto Retry**| PRD 5.3, 6.3 | 1초 후 자동 1회 재시도, 최종 실패 시 텍스트 유지 & 에러 문구 | 🔲 대기 |
| **10초 타임아웃 강제 중단** | PRD 5.4, 6.3 | 10초 초과 시 Abort, 로딩 해제, 텍스트 보존 & 타임아웃 문구 | 🔲 대기 |
| **AI 가드레일 안내** | PRD 5.5, 6.3 | `INVALID_INPUT` 수신 시 가드레일 안내 카드 노출 & 포커스 유지 | 🔲 대기 |
| **JSON 파싱 오류 복구** | PRD 5.6, 6.3 | 파싱 에러 시 자동 1회 재요청 및 사용자 안내 | 🔲 대기 |
| **초경량 클라이언트 동작** | PRD 1.2, 6.4 | 로그인/결제/외부 DB 없이 프론트엔드 단독 환경 완벽 동작 | 🔲 대기 |
